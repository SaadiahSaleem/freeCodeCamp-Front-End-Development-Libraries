...
2. The `Container` component should be wrapped by a `Provider`
3. The `Presentational` component should render to page.
4. The `Presentational` component should render an `h2`, `input`, `button`, and `ul` elements.
5. The `Presentational` component should receive `messages` from the Redux store as a prop.
6. The `Presentational` component should receive the `submitMessage` action creator as a prop.