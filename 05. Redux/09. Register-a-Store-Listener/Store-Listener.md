...
4. The function passed to `store.subscribe` should not be called.
5. The callback to `store.subscribe` should also increment the global `count` variable as the store is updated.